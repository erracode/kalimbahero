            ...
            <div className="flex flex-col gap-6">
              <div className="flex items-center justify-between">
                ...
                <div className="flex items-center gap-3">
                  ...
                  >
                    <Heart className={cn("w-4 h-4 skew-x-[12deg]", song.isFavorited && "fill-cyan-500")} />
                    <span className="skew-x-[12deg] text-xs font-black italic uppercase tracking-wider uppercase">BOOKMARK</span>
                  </button>
                </div>
              </div>
              ...
              <div className="flex items-center gap-2">
                {[1, 2, 3, 4, 5].map((s) => (
                  <button key={s} onClick={() => handleRate(s)} className="group cursor-pointer">
                    <Star className={cn("w-6 h-6 transition-all", s <= (song.userRating || song.averageRating || 0) ? "text-yellow-500 fill-yellow-500" : "text-white/10 group-hover:text-yellow-500/50")} />
                  </button>
                ))}
                <span className="ml-3 text-sm font-bold text-white/60">
                  {song.averageRating ? Number(song.averageRating).toFixed(1) : "NR"} (12 votes)
                </span>
              </div>
            </div>
