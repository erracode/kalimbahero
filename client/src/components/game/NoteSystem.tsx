...
import { getKeyColor, getLaneXPosition } from '@/utils/frequencyMap';
...
export const NoteSystem: React.FC<NoteSystemProps> = ({
  ...
  useFrame(() => {
    ...
    for (let i = 0; i < MAX_NOTES; i++) {
      if (i < notes.length) {
        const note = notes[i];
        const noteId = note.id || `temp_${i}`;
        const isHit = hitNotes.has(noteId);
        ...
        if (isVisible) {
          ...
          const x = getLaneXPosition(note.keyIndex, totalLanesCount);
          const color = getKeyColor(note.keyIndex, totalLanesCount);
